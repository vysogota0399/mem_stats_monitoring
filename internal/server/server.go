...
func (s *Server) signer() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		cms := crypto.NewCms(sha256.New(), s.secretKey)
		sign, err := signFromHeaders(c.Request)
		if err != nil {
...
func signFromHeaders(r *http.Request) ([]byte, error) {
	base64sign := r.Header.Get(signHeaderKey)
	if base64sign == "" {
		return nil, errors.New("sign not found in headers")
	}
